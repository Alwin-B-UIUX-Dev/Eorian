...
import { MaskerHelper } from '@/utils/MaskerHelper';
...
export class PostgresUserRepository implements IUserRepository {
  ...
  public async findAll(limit: number = 10, offset: number = 0): Promise<IUser[]> {
    try {
      ...
      const mappedUserData: IUserData[] = DatabaseMapper.snakeToCamelArray<IUserData>(usersData);

      // Conversion en objets User
      const users: IUser[] = mappedUserData.map(userData => new User(userData));
